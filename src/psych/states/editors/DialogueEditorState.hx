...
class DialogueEditorState extends MusicBeatState
{
  ...
  function reloadText(skipDialogue:Bool)
  {
    ...
    DiscordClient.instance.changePresence({details: "Dialogue Editor", state: rpcText});
    #end
  }
  ...
  function changeText(add:Int = 0)
  {
    ...
    if (leLength > 0)
    {
      ...
      character.playAnim(character.jsonFile.animations[curAnim].anim, daText.finishedText);
      animText.text = 'Animation: ' + character.jsonFile.animations[curAnim].anim
        + ' ('
        + (curAnim + 1)
        + ' / '
        + leLength
  ...
  function onLoadComplete(_):Void
  {
    ...
    if (fullPath != null)
    {
      ...
      if (rawJson != null)
      {
        var loadedDialog:DialogueFile = tjson.TJSON.parse(rawJson);
