...
class NotesSubState extends MusicBeatSubstate
{
  ...
  public function new()
  {
    ...
    #if DISCORD_ALLOWED
    DiscordClient.instance.changePresence({details: "Note Colors Menu"});
    #end
    ...
    var grid:FlxBackdrop = new FlxBackdrop(FlxGridOverlay.createGrid(80, 80, 160, 160, true, 0xF04B0298, 0x0));
    grid.velocity.set(40, 40);
    grid.alpha = 0;
    FlxTween.tween(grid, {alpha: 1}, 0.5, {ease: FlxEase.quadOut});
    add(grid);

    var ax:FlxSprite = new FlxSprite(-10);
    ax.y = 200;
    ax.frames = Paths.getSparrowAtlas('ax');
    ax.animation.addByPrefix('ax', 'ax', 24, true);
    ax.animation.play('ax');
    ax.antialiasing = ClientPrefs.data.antialiasing;
    ax.updateHitbox();
    ax.scrollFactor.set();
    add(ax);

    var aex:FlxSprite = new FlxSprite(10);
    aex.y = 350;
    aex.frames = Paths.getSparrowAtlas('aex');
    aex.animation.addByPrefix('aex', 'aex', 24, true);
    aex.animation.play('aex');
    aex.antialiasing = ClientPrefs.data.antialiasing;
    aex.updateHitbox();
    aex.scrollFactor.set();
    add(aex);
    ...
    var tipX = 20;
    var tipY = 660;
    var tip:FlxText = new FlxText(tipX, tipY, 0, "", 16);
    tip.setFormat(Paths.font("HelpMe.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
    tip.borderSize = 2;
    add(tip);

    tipTxt = new FlxText(tipX, tipY + 24, 0, '', 16);
    tipTxt.setFormat(Paths.font("HelpMe.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
    tipTxt.borderSize = 2;
    add(tipTxt);
  ...
  override function update(elapsed:Float)
  {
    ...
    @:privateAccess {
      ...
      {
        if (OptionsState.vcrEffect != null) OptionsState.vcrEffect.update(elapsed);
      }
    }
